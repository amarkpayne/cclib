...
class Mopac(logfileparser.Logfile):
    ...
    def extract(self, inputfile, line):
        ...
        if line.find("Empirical Formula:") > -1:
            ...
            natom = int(line.split()[
                            -2])  # second to last component should be number of atoms (last element is "atoms" (or possibly "atom"?))
        ...
        if (line.find("NUMBER    SYMBOL      (ANGSTROMS)     (ANGSTROMS)     (ANGSTROMS)") > -1 or line.find(
            "NUMBER   SYMBOL      (ANGSTROMS)     (ANGSTROMS)     (ANGSTROMS)") > -1):
            ...
            while len(
                line.split()) > 6:  # MOPAC Version 14.019L 64BITS suddenly appends this block with  "CARTESIAN COORDINATES" block with no blank line.
                broken = line.split()
        ...
        if line[0:35] == '          FINAL HEAT OF FORMATION =':
            if not hasattr(self, "scfenergies"):
                self.scfenergies = []
            self.scfenergies.append(utils.convertor(self.float(line.split()[5]) / 627.5095, "hartree",
